...
def inverse_document_frequencies(tokenized_documents):
    all_tokens_set = {}
    for url, doc in tokenized_documents.items():
        for w in doc:
...
def compute_tfidf(dict_documents):
    ...
    for url, document in dict_documents.items():
        for w in document:
    ...
    for token, list_tfidf in list_tfidf_in_all_documents.items():
        # print(list_tfidf)
...
def process_urls_content(url_content_folder, parent):
    ...
    for fn in documents:
        ...
        fin = open(p, 'r', encoding='utf-8', errors='ignore')
        url_name = fn
    ...
    fout = open(out_top_8K_words_file, 'w', encoding='utf-8')
    cnt = 0
...
def sentence_tokenize(infoloder='webscrapedtext', outfolder='sentences_tokenized'):
    ...
    fin1 = open('out_top_16K_words_file.txt', 'r', encoding='utf-8', errors='ignore')
    for line in fin1:
        _, w, _ = line.split()
    ...
    for fn in documents:
        p = join(infoloder, fn)
        fin = open(p, 'r', encoding='utf-8', errors='ignore')
        fin2 = open(join(infolder2, '%s.html' % fn.split('.')[0]), 'r', encoding='utf-8', errors='ignore')
        ...
        fout = open('%s/%s' % (outfolder, fn), 'w', encoding='utf-8')
        fout.write('%s\n' % url_name)
...
def divide_dataset_into_5parts(infolder= 'sentences_tokenized', outfolder='train_test_data'):
    ...
    s1 = list(zip(df1['snopes_page'], df1['claim_label']))
    s2 = list(zip(df2['snopes_page'], df2['claim_label']))
    # print(s1, len(s1))
    # print(s2, len(s2))
...
def process_wordcloud():
    fin = open('out_top_16K_words_file.txt', 'r', encoding='utf-8', errors='ignore')
    fout = open('top16K_words.csv', 'w', encoding='utf-8')
